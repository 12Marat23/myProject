...
а остальные - строчными, точно так же, как вы видите в газете.
...
* следует также работать с начальными и конечными пробелами и заглавными буквами.
...
Функция должна возвращать среднее значение между средним значением первых x 
элементов массива и средним значением последних y элементов массива.

Среднее значение должно быть вычислено, если и x, и y имеют значения больше 1, 
но меньше или равны длине массива. В противном случае функция должна возвращать значение -1.
...
# @test.describe("The mean of two means")
# def the_mean_of_two_means():
#     @test.it("Basic tests")
#     def basic_tests():
...
'''В этом Ката вам будет предоставлен массив целых чисел, 
элементы которого имеют как отрицательное, так и положительное значение, 
за исключением одного целого числа, которое является либо только отрицательным, 
либо только положительным. Ваша задача будет состоять в том, чтобы найти это целое число.
...
(единственное положительное или только отрицательное целое число может появляться более одного раза)
...
def sample_tests():
    ...
    def _():
        for arr, expected in sample_test_cases:
            msg = f'solve({arr})'
            test.assert_equals(solve(arr), expected, msg)
